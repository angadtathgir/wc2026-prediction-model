...
> **Archived — this is part one.** Written before the knockout rounds, with every coefficient hand-picked and nothing validated. It gave Spain 32.0%, and Spain went on to beat Argentina 1-0 in the final.
>
> That is not evidence the model worked. In [**part two**](index.html) I refit every parameter by maximum likelihood on 49,547 matches, backtested walk-forward across four World Cups, and found that three of the five layers below do not survive validation - including the one this post is built around. The host-advantage assumption here is wrong by a factor of about nineteen.
>
> This page is left up unedited, because the comparison is the point.
